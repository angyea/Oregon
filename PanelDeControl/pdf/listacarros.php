...
class MiPDF extends FPDF {
	...
	$cabeceraT = array("Codigo ");
		...
		$mipdf -> Ln (2);
		$mipdf -> Cell(200,10,"Lista De Carros $fecha",0,0,'C');
		$mipdf -> Ln (10);
		...
		for ($i = 0; $i < count($cabeceraT); $i++)
		{
			...
			$mipdf -> SetFont('ARIAL','B', 9);
$mipdf -> SetFillColor(0, 191, 255);
	$mipdf -> Cell(35,11,"Placa",1,0,'C',true);
	$mipdf -> SetFont('ARIAL','B', 9);
$mipdf -> SetFillColor(0, 191, 255);
	$mipdf -> Cell(30,11,"Marca",1,0,'C',true);

		$mipdf -> SetFont('ARIAL','B', 9);
$mipdf -> SetFillColor(0, 191, 255);
	$mipdf -> Cell(30,11,"Color",1,0,'C',true);
		...
		$mipdf -> SetFont('ARIAL','B', 9);
$mipdf -> SetFillColor(0, 191, 255);
	$mipdf -> Cell(60,11,"Tipo",1,0,'C',true);
...
$num = 0; 
	...
	while ( $datos = $bd-> mostrar_registros($sql2))
	{
		...
		$nombre = $datos ['placa'];
		$apellido = $datos ['marca'];
		
		$apellidos = $datos ['color'];
		$p= $datos ['tipo'];
