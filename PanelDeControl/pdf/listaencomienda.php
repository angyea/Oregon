...
class MiPDF extends FPDF {
	...
	$cabeceraT = array("Codigo ");
		...
		$mipdf -> Ln (2);
		$mipdf -> Cell(200,10,"Lista De Encomiendas $fecha",0,0,'C');
		$mipdf -> Ln (10);
		...
		for ($i = 0; $i < count($cabeceraT); $i++)
		{
			...
			$mipdf -> SetFont('ARIAL','B', 9);
$mipdf -> SetFillColor(0, 191, 255);
	$mipdf -> Cell(35,11,"Codigo",1,0,'C',true);
	$mipdf -> SetFont('ARIAL','B', 9);
$mipdf -> SetFillColor(0, 191, 255);
	$mipdf -> Cell(30,11,"Origen",1,0,'C',true);

		$mipdf -> SetFont('ARIAL','B', 9);
$mipdf -> SetFillColor(0, 191, 255);
	$mipdf -> Cell(30,11,"Destino",1,0,'C',true);
		...
		$mipdf -> SetFont('ARIAL','B', 9);
$mipdf -> SetFillColor(0, 191, 255);
	$mipdf -> Cell(60,11,"Destinatario",1,0,'C',true);
...
$sql="SELECT * FROM encomienda where id_encomienda";
...
$num = 0; 
	...
	while ( $datos = $bd-> mostrar_registros($sql2))
	{
		...
		$nombre = $datos ['codigo'];
		$apellido = $datos ['origen'];
		
		$apellidos = $datos ['destino'];
		$p= $datos ['destinatario'];
