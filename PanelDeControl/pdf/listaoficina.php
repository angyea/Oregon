...
class MiPDF extends FPDF {
	...
	$cabeceraT = array("Codigo ");
		...
		$mipdf -> Ln (2);
		$mipdf -> Cell(200,10,"Lista De Oficinas $fecha",0,0,'C');
		$mipdf -> Ln (10);
...
$mipdf -> SetFillColor(0, 191, 255);
	...
	$mipdf -> SetFont('ARIAL','B', 9);
$mipdf -> SetFillColor(0, 191, 255);
	$mipdf -> Cell(30,11,"Razon Social",1,0,'C',true);

		$mipdf -> SetFont('ARIAL','B', 9);
$mipdf -> SetFillColor(0, 191, 255);
	$mipdf -> Cell(60,11,"Direccion",1,0,'C',true);
		...
		$mipdf -> SetFont('ARIAL','B', 9);
$mipdf -> SetFillColor(0, 191, 255);
	$mipdf -> Cell(60,11,"Telefono",1,0,'C',true);
...
$sql="SELECT * FROM oficina where id_oficina";
...
$num = 0; 
	...
	while ( $datos = $bd-> mostrar_registros($sql2))
	{
		...
		$nombre = $datos ['numero'];
		$apellido = $datos ['razon'];
		
		$apellidos = $datos ['direccion'];
		$p= $datos ['telefono'];
  ...
  $num++;  
	   ...
	   $mipdf -> Cell(10,5,"$num",1,0,'C');
		...
		$mipdf -> Cell(30,5,"$apellido",1,0,'C');	
		  ...
		  $mipdf -> Cell(60,5,"$apellidos",1,0,'C');
